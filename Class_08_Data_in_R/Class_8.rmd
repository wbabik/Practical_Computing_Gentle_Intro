...
> description).  
> Another reason for using argument names is code readability,
...
numbers typed as strings (e.g., "2"). What is the outcome? A vector of integers or a vector of strings? Why?
...
Data frame is something you probably know as "table", similar to, e.
g., spreadsheet table. Data frame has, however, some unique characteristics:
...
windows click on a column header to sort the data according to the values in a
...
Column type can also be checked with `typeof()` or `class()`
